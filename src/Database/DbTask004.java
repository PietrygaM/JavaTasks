 ...
 * The problem is, you don't know who you are working with and your predecessor 
 * has vanished without a trace! Luckily, each project in your company keeps its own 
 * activity database, which you are going to use to find out the names of your new colleagues.

Information about the project's activity is stored in table projectLog, which has the 
following structure:
...
You've decided that finding everyone who interacted with the project in this period is the best 
way to start.

Given the table projectLog, build a new results table with a single name column that contains 
the names of the project's contributors sorted in ascending order.
...
public class DbTask004 {

	String query = "SELECT DISTINCT name from projectLog ORDER BY name ASC;";
