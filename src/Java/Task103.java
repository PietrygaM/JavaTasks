...
public class Task103 {
	...
	public static boolean isCryptSolution(String[] crypt, char[][] solution) {
		Float[] result = new Float[crypt.length];
		for (int i = 0; i < crypt.length; i++) {
			char[] tmpCrypt = crypt[i].toCharArray();
			String tmpResult = "";
			for (int k = 0; k < tmpCrypt.length; k++) {
				for (int j = 0; j < solution.length; j++) {
					if (tmpCrypt[k] == solution[j][0]) {
						if (k == 0 && tmpCrypt.length>1 && solution[j][1] == '0') {
							return false;
						} else {
							tmpResult += solution[j][1];
							continue;
						}
					}
				}
			}
			result[i] = Float.parseFloat(tmpResult);
		}
